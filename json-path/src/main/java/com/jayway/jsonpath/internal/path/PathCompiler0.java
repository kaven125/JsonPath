...
public class PathCompiler0 {
  ...
  private PathCompiler0(String path, LinkedList<Predicate> filterStack) {
    this(new CharacterIndex(path), filterStack);
  }

  private PathCompiler0(CharacterIndex path, LinkedList<Predicate> filterStack) {
  ...
  public static Path compile(String path, final Predicate... filters) {
    try {
      ...
      return new PathCompiler0(ci, filterStack).compile();
    } catch (Exception e) {
    ...
    if (isFunction) {
      ...
      if (path.inBounds(readPosition + 1)) {
        ...
        if (c != CLOSE_PARENTHESIS) {
          ...
          // parse the arguments of the function - arguments that are inner queries or JSON
          // document(s)
          String functionName = path.subSequence(startPosition, endPosition).toString();
   ...
   * caller has supplied another path expression which must be evaluated and in turn invoked against
   * the root document. In this tokenizer we're only concerned with parsing the path thus the output
   * of this function is a list of parameters with the Path set if the parameter is an expression.
   * If the parameter is a JSON document then the value of the cachedValue is set on the object.
   *
   * Sequence for parsing out the parameters:
   *
   * This code has its own tokenizer - it does some rudimentary level of lexing in that it can
   * distinguish between JSON block parameters and sub-JSON blocks - it effectively regex's out the
   * parameters into string blocks that can then be passed along to the appropriate parser. Since
   * sub-jsonpath expressions can themselves contain other function calls this routine needs to be
   * sensitive to token counting to determine the boundaries. Since the Path parser isn't aware of
   * JSON processing this uber routine is needed.
   *
   ...
   * </pre>
   *
   * The above is a valid function call, we're first summing 10 + avg of 1...10 (5.5) so the total
   * should be 15.5
   *
   * @return An ordered list of parameters that are to processed via the function. Typically
   *         functions either process an array of values and/or can consume parameters in addition
   *         to the values provided from the consumption of an array.
   */
  private List<Parameter> parseFunctionParameters(String funcName) {
    ParamType type = null;

    // Parenthesis starts at 1 since we're marking the start of a function call, the close paren
    // will denote the
    ...
    while (path.inBounds() && !endOfStream) {
      ...
      if (type == null) {
        ...
        if (c == OPEN_BRACE || isDigit(c) || DOUBLE_QUOTE == c || MINUS == c) {
          ...
          type = ParamType.PATH; // read until we reach a terminating comma and we've reset grouping
                                 // to zero
        }
      }
      ...
      switch (c) {
        ...
        // In either the close paren case where we have zero paren groups left, capture the
        // parameter, or where
        ...
        case COMMA:
          // In this state we've reach the end of a function parameter and we can pass along the
          // parameter string
          // to the parser
            ...
            if (null != type) {
              ...
              switch (type) {
                ...
                case PATH:
                  ...
                  PathCompiler0 compiler = new PathCompiler0(parameter.toString(), predicates);
